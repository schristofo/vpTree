 ...
 * Description:
 *  This is the parallel implementation of the Vantage Point Tree problem using the OpenMP library.
...
vptree * subTreeRec(double * X, int * idx, int n, int d) {
  ...
  #pragma omp parallel for schedule(static,10) num_threads (4)//schedule(static,250000) num_threads(8)
  for (int i = 0; i < n-1; i++) {
